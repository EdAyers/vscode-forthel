...
import { Connection, Transport, TransportError, Event } from './server';
import { DEFAULT_EXECUTABLE_PATH } from './constants';
...
export class ProcessTransport implements Transport {
    ...
    constructor(executablePath: string, workingDirectory: string, options: string[]) {
        this.executablePath = executablePath || DEFAULT_EXECUTABLE_PATH;
    ...
    connect(): Connection {
        ...
        process.on('error', (e) => {
            ...
            conn.error.fire({error: 'connect', reason: 'process-startup',
                message: `Unable to start the server process: ${e}`});
        });
